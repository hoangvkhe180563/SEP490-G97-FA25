...
const ColorBarChart: React.FC<ColorBarChartProps> = ({
  ...
  const barW = Math.max(minBarWidth, Math.floor(900 / Math.max(1, data.length)) - 12);
      ...
      >
        <g transform={`translate(${margin.left}, ${margin.top})`}>
          {[0, 0.25, 0.5, 0.75, 1].map((t, i) => {
            ...
            return (
              <line key={i} x1={0} x2={svgWidth} y1={y} y2={y} stroke="#eef2ff" strokeWidth={1} />
            );
          })}
            ...
            return (
              <g key={i}>
                ...
                <text
                  ...
                  fill="#1f2937"
                  fontWeight={700}
                  transform={rotateLabels ? `rotate(-45 ${labelX} ${labelY})` : undefined}
                >
                  {formatLabel(d.label)}
                </text>
              </g>
...
const DashboardPage: React.FC = () => {
  ...
  const genderRatio = useDashboardStore((s) => (s as any).genderRatio) as KeyValueDto[];
  ...
  const schoolName = useDashboardStore((s) => (s as any).schoolName) as string | null;
  ...
  const fetchMySchool = useDashboardStore((s) => (s as any).fetchMySchool) as () => Promise<string | null>;
    ...
    <div className="p-5 space-y-5 w-full h-full overflow-y-auto text-sm">
      <header className="flex items-start justify-between gap-6">
        <div>
          ...
          <div className="mt-2">
            <div className="text-lg font-semibold text-slate-800" aria-label="school-name">
              {schoolName ?? "—"}
            </div>
        ...
        <div className="flex items-center gap-2">
          <Button onClick={() => handleTabClick(1)} className={`${headerButtonBase} ${tab === 1 ? "bg-indigo-600 text-white" : "bg-slate-800 text-white/90"}`}>
            Tổng quan
          </Button>
          <Button onClick={() => handleTabClick(2)} className={`${headerButtonBase} ${tab === 2 ? "bg-rose-500 text-white" : "bg-slate-800 text-white/90"}`}>
            Học sinh & Giáo viên
          </Button>
          <Button onClick={() => handleTabClick(3)} className={`${headerButtonBase} ${tab === 3 ? "bg-emerald-600 text-white" : "bg-slate-800 text-white/90"}`}>
            Hoạt động lớp
          </Button>
          <Button onClick={() => handleTabClick(4)} className={`${headerButtonBase} ${tab === 4 ? "bg-sky-600 text-white" : "bg-slate-800 text-white/90"}`}>
            Bài tập
          </Button>
        </div>
      </header>
        ...
        <MetricCard
          ...
          value={String(
            (safeArr(studentsPerClass, []).length > 0
              ? safeArr(studentsPerClass, []).reduce((a, c) => a + (c.students ?? 0), 0) / safeArr(studentsPerClass, []).length
              : 0
            ).toFixed(1)
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <main className="lg:col-span-2 space-y-5">
            ...
            <>
              <Card>
                <CardHeader className="p-4 flex items-center justify-between">
                  <div>
                    <CardTitle className="text-lg font-semibold">Số lớp theo khối</CardTitle>
                    <div className="text-xs text-slate-500">Phân bố lớp theo khối</div>
                  </div>
                  <div className="text-sm text-slate-600">{isLoading ? "Đang tải..." : "Dữ liệu"}</div>
                </CardHeader>
                <CardContent className="p-4 pt-0">
                  <ColorBarChart
                    data={safeArr(classesByGrade, []).map((g) => ({
                      label: String((g as any).grade ?? ""),
                      value: typeof (g as any).count === "number" ? (g as any).count : Number((g as any).Count ?? 0),
                      color: `hsl(${Number((g as any).grade ?? 0) * 36},70%,45%)`,
                    }))}
                    height={180}
                  />
                </CardContent>
              ...
              <Card>
                <CardHeader className="p-4 flex items-center justify-between">
                  <div>
                    <CardTitle className="text-lg font-semibold">Trung bình điểm theo lớp</CardTitle>
                    <div className="text-xs text-slate-500">Trung bình điểm (Avg) từ các submission đã chấm</div>
                  </div>
                  <div className="text-sm text-slate-600">{classAverages.length} lớp</div>
                </CardHeader>
                <CardContent className="p-4 pt-0">
                  {classAverages.length === 0 ? (
                    ...
                    <ColorBarChart
                      data={safeArr(classAverages, []).map((c, i) => ({
                        ...
                        value: Number(Math.round((c.avgScore ?? 0) * 100) / 100),
                        color: ["#60a5fa", "#34d399", "#f59e0b", "#f472b6", "#a78bfa"][i % 5],
                      }))}
                      height={220}
                      minBarWidth={60}
                    />
              ...
              <Card>
                <CardHeader className="p-4 flex items-center justify-between">
                  <div>
                    <CardTitle className="text-lg font-semibold">Thống kê theo tháng</CardTitle>
                    <div className="text-xs text-slate-500">Số liệu theo tháng</div>
                  </div>

                  <div className="flex items-center gap-3">
                    <div className="text-sm text-slate-600">{monthsToShow} tháng {newClassesYear ? `— Năm ${newClassesYear}` : ""}</div>

                    <Select value={String(monthsToShow)} onValueChange={(v: any) => setMonthsToShow(Number(v))}>
                ...
                <CardContent className="p-4 pt-0 space-y-4">
                  <div>
                    <div className="text-sm font-medium text-slate-700 mb-2">Bài tập</div>
                    <ColorBarChart data={safeArr(monthlyClassworks, []).map((m) => ({ label: m.month, value: m.count }))} height={200} monthOnlyLabels minBarWidth={40} />
                  </div>

                  <div>
                    <div className="text-sm font-medium text-slate-700 mb-2">Thông báo</div>
                    <ColorBarChart data={safeArr(monthlyNotifications, []).map((m) => ({ label: m.month, value: m.count }))} height={200} monthOnlyLabels minBarWidth={40} />
                  </div>

                  <div>
                    <div className="text-sm font-medium text-slate-700 mb-2">Lượt nộp</div>
                    <ColorBarChart data={safeArr(monthlySubmissions, []).map((m) => ({ label: m.month, value: m.count }))} height={200} monthOnlyLabels minBarWidth={40} />
                  </div>

                  <div>
                    <div className="text-sm font-medium text-slate-700 mb-2">Lớp mới</div>
                    <ColorBarChart data={safeArr(monthlyNewClasses, []).map((m) => ({ label: m.month, value: m.count }))} height={200} monthOnlyLabels minBarWidth={40} />
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="p-4">
                  <CardTitle className="text-lg font-semibold">Top lớp theo số học sinh (Top 5)</CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-0">
                  <ColorBarChart data={safeArr(studentsPerClass, []).slice(0, 5).map((c, i) => ({ label: c.className || `Lớp ${c.classId}`, value: c.students, color: ["#60a5fa", "#f472b6", "#34d399", "#f59e0b", "#a78bfa"][i % 5] }))} height={160} />
                </CardContent>
              </Card>
          ...
          {tab === 2 && (
            <>
              <Card>
                <CardHeader className="p-4">
                  <CardTitle className="text-lg font-semibold">Tỷ lệ giới tính</CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-0 flex gap-4 items-start">
                  <Donut data={safeArr(genderRatio, []).map((g) => ({ label: g.key, value: g.value, color: g.key === "Male" ? "#2563eb" : "#ec4899" }))} size={140} />
                  <div>
                    {safeArr(genderRatio, []).map((g, idx) => {
                      ...
                      return (
                        <div key={idx} className="mb-2 flex items-center gap-3 text-sm">
                          <div style={{ background: g.key === "Male" ? "#2563eb" : "#ec4899" }} className="w-3 h-3 rounded" />
                          <div>{g.key}: <span className="font-semibold">{value}</span> ({pct}%)</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <Card>
                <CardHeader className="p-4">
                  <CardTitle className="text-lg font-semibold">Tỷ lệ xác minh email</CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-0 flex gap-4 items-start">
                  <Donut data={safeArr(emailVerified, []).map((e) => ({ label: e.key, value: e.value, color: e.key === "Verified" ? "#10b981" : "#f97316" }))} size={140} />
                  <div>
                    {safeArr(emailVerified, []).map((e, idx) => {
                      ...
                      return (
                        <div key={idx} className="mb-2 flex items-center gap-3 text-sm">
                          <div style={{ background: e.key === "Verified" ? "#10b981" : "#f97316" }} className="w-3 h-3 rounded" />
                          <div>{e.key}: <span className="font-semibold">{value}</span> ({pct}%)</div>
                        </div>
                      );
                    })}
                  </div>
          ...
          {tab === 3 && (
            <>
              <Card>
                <CardHeader className="p-4">
                  <CardTitle className="text-lg font-semibold">Số thông báo theo loại</CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-0">
                  <ColorBarChart data={safeArr(announcementsByType, []).map((a) => ({ label: a.key, value: a.value, color: "#7c3aed" }))} height={160} />
                </CardContent>
              </Card>

              <Card>
                <CardHeader className="p-4">
                  <CardTitle className="text-lg font-semibold">Tỷ lệ đọc thông báo</CardTitle>
                </CardHeader>
              ...
              <Card>
                <CardHeader className="p-4">
                  <CardTitle className="text-lg font-semibold">Top lớp hoạt động (Top 5)</CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-0">
                        ...
                        {topActiveTop5.map((c, i) => (
                          <TableRow key={c.classId}>
                            ...
                            <TableCell className="text-sm">
                              <div className="flex items-center gap-3">
                                <div className={"w-6 h-2 rounded " + (i === 0 ? "bg-indigo-600" : i === 1 ? "bg-emerald-500" : "bg-amber-500")} />
              ...
              <Card>
                <CardHeader className="p-4">
                  <CardTitle className="text-lg font-semibold">Thông báo có nhiều tương tác nhất (Top 5)</CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-0 space-y-2">
                  {topReadTop5.map((n) => (
                    <div key={n.notificationId} className="flex justify-between border rounded p-3 text-sm">
                      <div>
                        <div className="font-medium">{n.title}</div>
                        {n.createdBy ? <div className="text-xs text-slate-500 mt-1">Bởi {n.createdBy}</div> : null}
                      </div>
                      <div className="text-sm text-slate-600">{n.readsCount} bình luận</div>
                    </div>
                  ))}
                </CardContent>
              </Card>
          ...
          {tab === 4 && (
            <>
              <Card>
                <CardHeader className="p-4 flex items-center justify-between">
                  <div>
                    <CardTitle className="text-lg font-semibold">Tỷ lệ nộp bài</CardTitle>
                    <div className="text-xs text-slate-500">Tổng quan nộp bài</div>
                  </div>
                  <div className="text-2xl font-extrabold text-emerald-600">{String(Math.round(submissionRateValue * 100)) + "%"}</div>
                </CardHeader>
                <CardContent className="p-4 pt-0">
              ...
              <Card>
                <CardHeader className="p-4">
                  <CardTitle className="text-lg font-semibold">Phân bố điểm</CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-0">
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
                    {safeArr(scoreDistribution, []).map((s, idx) => {
                      ...
                      return (
                        <div key={idx} className={"p-2 rounded border text-center " + bg}>
                          <div className="text-sm font-medium text-white">{range}</div>
                          <div className="mt-1 text-xl font-semibold text-white">{String(Math.round(pct * 100)) + "%"}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <Card>
                <CardHeader className="p-4">
                  <CardTitle className="text-lg font-semibold">Bài tập nhiều tương tác nhất (Top 5)</CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-0 space-y-2">
                  {mostInteractiveTop5.map((a, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between p-2 rounded border text-sm">
                        <div className="flex items-center gap-2">
                          <div className={"w-3 h-3 rounded " + (idx === 0 ? "bg-sky-500" : "bg-slate-400")} />
                          <div className="font-medium">{title}</div>
                          <div className="text-xs text-slate-500 ml-2">{a.createdBy}</div>
                        </div>
        ...
        <aside className="space-y-4">
          <Card>
            <CardHeader className="p-4">
              <CardTitle className="text-sm text-slate-500">Tổng quan hệ thống</CardTitle>
            </CardHeader>
            <CardContent className="p-4 pt-0 text-sm text-slate-700 space-y-1">
              <div>Tổng người dùng: <span className="font-semibold">{String(totalUsers)}</span></div>
              <div>Tổng lớp: <span className="font-semibold">{String(totalClasses)}</span></div>
              <div>Tổng bài tập: <span className="font-semibold">{String(totalAssignments)}</span></div>
            </CardContent>
          </Card>
          ...
          {classAverages && classAverages.length > 0 && (
            <Card>
              <CardHeader className="p-4">
                <CardTitle className="text-sm text-slate-500">Top lớp theo điểm trung bình</CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-0 text-sm text-slate-700">
                <div className="space-y-2">
                  {safeArr(classAverages, []).slice(0, 5).sort((a,b)=> b.avgScore - a.avgScore).map((c) => (
          ...
          {classWithMostNotifications && (
            <Card>
              <CardHeader className="p-4">
                <CardTitle className="text-sm text-slate-500">Lớp có nhiều thông báo nhất</CardTitle>
              </CardHeader>
              <CardContent className="p-4 pt-0 text-sm text-slate-700">
                <div className="font-medium text-sm">{classWithMostNotifications.className}</div>
                <div className="text-sm">Số thông báo: <span className="font-semibold">{classWithMostNotifications.notificationsCount}</span></div>
                <div className="text-sm">Số lượt nộp: <span className="font-semibold">{classWithMostNotifications.submissionsCount}</span></div>
              </CardContent>
            </Card>
          )}
        </aside>
