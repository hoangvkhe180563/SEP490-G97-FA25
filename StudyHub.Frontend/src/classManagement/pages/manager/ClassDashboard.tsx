      ...
      <CardContent className="p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-4xl md:text-6xl font-extrabold mt-3">
              {value}
            </div>
          </div>
          <div
            className={
              "w-16 h-16 md:w-20 md:h-20 rounded-lg bg-gradient-to-br " +
              accentClass +
...
const ColorBarChart: React.FC<ColorBarChartProps> = ({
  ...
  const barW = Math.max(
    minBarWidth,
    Math.floor(1000 / Math.max(1, data.length)) - 12
  );
...
const DashboardPage: React.FC = () => {
  ...
  ) as KeyValueDto[]; // may be undefined in store
  ...
  const headerButtonBase =
    "text-lg md:text-xl px-4 md:px-5 py-2 md:py-3 rounded-md";
    ...
    <div className="p-6 space-y-6 w-full h-full overflow-y-auto">
      <header className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-2">
            <div
              className="text-2xl md:text-3xl font-semibold text-slate-800 leading-tight"
              aria-label="school-name"
            >
              {schoolName ?? "—"}
            </div>
            <div className="text-sm text-slate-500 mt-1">Trường</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            className={`${headerButtonBase} ${
              tab === 4 ? "bg-sky-600 text-white" : "bg-slate-900 text-white/90"
            }`}
          >
            Bài tập
          </Button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <MetricCard
          ...
          icon={<User className="text-white" size={18} strokeWidth={1.6} />}
        />
        <MetricCard
          title="Tổng bài tập / thông báo"
          value={`${totalAssignments} / ${totalAnnouncements}`}
          accent="from-sky-500 to-indigo-500"
          icon={<BookOpen className="text-white" size={18} strokeWidth={1.6} />}
        />
        <MetricCard
          ...
          icon={<Users className="text-white" size={18} strokeWidth={1.6} />}
        />
        <MetricCard
          title="Tỷ lệ nộp bài"
          value={`${Math.round(submissionRateValue * 100)}%`}
          accent="from-amber-400 to-amber-600"
          icon={<FileText className="text-white" size={18} strokeWidth={1.6} />}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <main className="lg:col-span-2 space-y-6">
            ...
            <>
              <Card>
                <CardHeader className="p-5 flex items-center justify-between">
                  <div>
                    <CardTitle className="text-3xl md:text-4xl">Số lớp theo khối</CardTitle>
                ...
                <CardContent className="p-5 pt-0">
                  <ColorBarChart
                    data={safeArr(classesByGrade, []).map((g) => ({
                      ...
                      color: `hsl(${Number((g as any).grade ?? 0) * 40},70%,50%)`,
                    }))}
                    height={200}
                  />
                </CardContent>
              ...
              <Card>
                <CardHeader className="p-5 flex items-center justify-between">
                  <div>
                    <CardTitle className="text-3xl md:text-4xl">Thống kê theo tháng</CardTitle>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-base md:text-lg text-slate-600">
                      {monthsToShow} tháng {newClassesYear ? `— Năm ${newClassesYear}` : ""}
                    </div>
                ...
                <CardContent className="p-5 pt-0 space-y-6">
                  ...
                  <div>
                    <div className="text-xl md:text-2xl font-medium text-slate-700 mb-2">Lớp mới</div>
              ...
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Top lớp theo số học sinh (Top 5)</CardTitle>
                </CardHeader>
                <CardContent className="p-5 pt-0">
                  <ColorBarChart data={safeArr(studentsPerClass, []).slice(0, 5).map((c, i) => ({ label: c.className || `Lớp ${c.classId}`, value: c.students, color: ["#60a5fa", "#f472b6", "#34d399", "#f59e0b", "#a78bfa"][i % 5] }))} height={200} />
                </CardContent>
              </Card>

              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Tổng quan lớp</CardTitle>
                </CardHeader>
                <CardContent className="p-5 pt-0">
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead>Tỷ lệ nộp</TableHead>
                          <TableHead>Tỷ lệ đọc</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {safeArr(classStats, []).slice(0, 10).map((cs) => (
                          <TableRow key={cs.classId}>
                            ...
                            <TableCell>{cs.studentsCount}</TableCell>
                            <TableCell>{String(Math.round(cs.submissionRate * 100)) + "%"}</TableCell>
                            <TableCell>{String(Math.round(cs.readRate * 100)) + "%"}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
          ...
          {tab === 2 && (
            <>
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Tỷ lệ giới tính</CardTitle>
                </CardHeader>
                <CardContent className="p-5 pt-0 flex gap-6 items-start">
              ...
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Tỷ lệ xác minh email</CardTitle>
                </CardHeader>
                <CardContent className="p-5 pt-0 flex gap-6 items-start">
          ...
          {tab === 3 && (
            <>
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Số thông báo theo loại</CardTitle>
                </CardHeader>
                <CardContent className="p-5 pt-0">
              ...
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Tỷ lệ đọc thông báo</CardTitle>
                </CardHeader>
              ...
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Top lớp hoạt động (Top 5)</CardTitle>
                </CardHeader>
                <CardContent className="p-5 pt-0">
              ...
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Thông báo được đọc nhiều nhất (Top 5)</CardTitle>
                </CardHeader>
                  ...
                  {topReadTop5.map((n) => (
                    <div key={n.notificationId} className="flex justify-between border rounded p-3">
                      <div>
                        ...
                        {n.createdBy ? <div className="text-sm text-slate-500 mt-1">Bởi {n.createdBy}</div> : null}
                      </div>
              ...
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Thông báo bị bỏ qua nhiều nhất (Top 5)</CardTitle>
                </CardHeader>
                <CardContent className="p-5 pt-0 space-y-2">
          ...
          {tab === 4 && (
            <>
              <Card>
                <CardHeader className="p-5 flex items-center justify-between">
                  <div>
                    <CardTitle className="text-3xl md:text-4xl">Tỷ lệ nộp bài</CardTitle>
              ...
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Phân bố điểm</CardTitle>
                </CardHeader>
                <CardContent className="p-5 pt-0">
              ...
              <Card>
                <CardHeader className="p-5">
                  <CardTitle className="text-3xl md:text-4xl">Bài tập nhiều tương tác nhất (Top 5)</CardTitle>
                </CardHeader>
                <CardContent className="p-5 pt-0 space-y-2">
                      ...
                      <div key={idx} className="flex items-center justify-between p-3 rounded border">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="font-medium text-base md:text-lg">{title}</div>
                           <div className="font-light text-base md:text-lg">{a.createdBy}</div>
                        </div>
        ...
        <aside className="space-y-6">
          <Card>
            <CardHeader className="p-5">
              <CardTitle className="text-base md:text-lg text-slate-500">Tổng quan hệ thống</CardTitle>
            </CardHeader>
            <CardContent className="p-5 pt-0 text-base md:text-lg text-slate-700 space-y-2">
          ...
          {classWithMostNotifications && (
            <Card>
              <CardHeader className="p-5">
                <CardTitle className="text-base md:text-lg text-slate-500">Lớp có nhiều thông báo nhất</CardTitle>
              </CardHeader>
              <CardContent className="p-5 pt-0 text-base md:text-lg text-slate-700">
