      ...
      <div className="flex-1">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: c.content ?? c.html ?? c.text ?? "",
          }}
        />
      </div>
    </div>
...
const ClassworkDetail: React.FC = () => {
  ...
  const classIdResolved =
    ...
    (() => {
      ...
      if (idx !== -1) {
        ...
        if (segs.length > idx + 2 && !isNaN(Number(segs[idx + 2])))
          return segs[idx + 2];
        // or pattern: ["class", "<classId>", ...]
        if (segs.length > idx + 1 && !isNaN(Number(segs[idx + 1])))
          return segs[idx + 1];
      }
      return undefined;
    })();
  ...
  // UI state
  const [classwork, setClasswork] = useState<ClassWork | null | undefined>(
    undefined
  );
  ...
  const [cwDetail, setCwDetail] = useState<null | {
    data?: any;
    submissions?: any[];
    files?: any[];
  }>(null);
  ...
  const worksLength =
    currentClass && currentClass.data && Array.isArray(currentClass.data.works)
      ? currentClass.data.works.length
      : 0;
  const teachersLength =
    currentClass &&
    currentClass.data &&
    Array.isArray(currentClass.data.teachers)
      ? currentClass.data.teachers.length
      : 0;
  ...
  const teacherForClasswork = useMemo(() => {
    ...
    return (
      (currentClass && currentClass.data && currentClass.data.teachers) ?? null
    );
  ...
  useEffect(() => {
    if (!classIdResolved) {
      console.debug(
        "ClassworkDetail: no classIdResolved, skipping initial fetch"
      );
      return;
    }
    ...
    (async () => {
      try {
        console.debug(
          "ClassworkDetail: fetching class info/works for classId",
          classIdResolved
        );
        if (typeof getClassInfo === "function")
          await getClassInfo(Number(classIdResolved));
      } catch (err) {
        console.error("getClassInfo failed", err);
      }
      try {
        if (typeof getClassWorks === "function")
          await getClassWorks(Number(classIdResolved));
      } catch (err) {
  ...
  useEffect(() => {
    const works =
      currentClass &&
      currentClass.data &&
      Array.isArray(currentClass.data.works)
        ? currentClass.data.works
        : [];
  ...
  useEffect(() => {
        ...
        try {
          if (typeof getClassworkDetail === "function") {
            ...
            if (detail) {
              ...
              if (
                Array.isArray(detail.submissions) &&
                detail.submissions.length > 0
              ) {
                const normalized: ClassworkSubmission[] =
                  detail.submissions.map((s: any) => ({
                    id: s.id,
                    classworkId:
                      s.classworkId ?? s.notificationId ?? classwork.id,
                    ...
                    files: (s.submissionFiles ?? s.files ?? []).map(
                      (f: any) => ({
                        id: f.id,
                        fileName: f.fileName,
                        fileUrl: f.fileUrl,
      ...
      (async () => {
        try {
          if (typeof getSubmissionByUserAndClasswork === "function") {
            const mine = await getSubmissionByUserAndClasswork(
              Number(classwork.id),
              currentUserId
            );
    ...
    const notiList =
      currentClass &&
      currentClass.data &&
      Array.isArray(currentClass.data.notifications)
        ? currentClass.data.notifications
        : [];
    const noti: ClassNotification | undefined = notiList.find(
      (n: any) => Number(n.id) === Number(classwork.id)
    );
  ...
  const handleAddLink = async (kind: "drive" | "link") => {
    const url = window.prompt(
      `${kind === "drive" ? "Drive URL" : "Nhập đường liên kết"}`
    );
    if (!url) return;
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if (created && typeof created === "object") {
        ...
        if (typeof getClassInfo === "function" && classIdResolved)
          void getClassInfo(Number(classIdResolved));
      } else {
        if (typeof getClassInfo === "function" && classIdResolved)
          await getClassInfo(Number(classIdResolved));
      }
  ...
  const classworkFiles: any[] =
    classwork.files && classwork.files.length > 0
      ? classwork.files
      : cwDetail?.files ?? [];
  const classworkLinks: any[] =
    classwork.links && classwork.links.length > 0
      ? classwork.links
      : cwDetail?.data?.links ?? cwDetail?.data?.linkDtos ?? [];
  ...
  const allowSubmissionFlag = (() => {
    ...
    if (Object.prototype.hasOwnProperty.call(cwAny, "allowSubmission"))
      return Boolean(cwAny.allowSubmission);
    if (Object.prototype.hasOwnProperty.call(cwAny, "allow_submission"))
      return Boolean(cwAny.allow_submission);
    if (Object.prototype.hasOwnProperty.call(cwAny, "allowSubmit"))
      return Boolean(cwAny.allowSubmit);
    return true;
  })();
    ...
    <div className="p-8 w-full h-full overflow-y-auto">
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-start gap-5">
          <Button
            variant="ghost"
            onClick={() =>
              navigate(`/class/${role}/${classIdResolved}?tab=exercise`)
            }
            className="p-2"
          >
            ←
          ...
          <div>
            ...
            <div className="text-base text-slate-500 mt-1">
              <span>
                {teacherForClasswork?.fullname ?? "Giáo viên"} •{" "}
      ...
      <div className="grid grid-cols-12 gap-6">
        <div className="col-span-12 lg:col-span-8">
          <Card className="p-6">
            <div className="text-slate-700 mb-6 text-lg">
              {classwork.description || "Không có mô tả"}
            </div>
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-sm text-slate-500">
                  {(classworkFiles?.length ?? 0) +
                    (classworkLinks?.length ?? 0)}{" "}
                  mục
                </div>
              </div>

              {(classworkFiles && classworkFiles.length > 0) ||
              (classworkLinks && classworkLinks.length > 0) ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  {classworkLinks?.map((l: any, idx: number) => (
                    <div
                      key={`link-${idx}`}
                      className="p-3 bg-white border rounded-lg flex items-center gap-3"
                    >
                ...
                <div className="text-sm text-slate-500">
                  Không có tài liệu kèm theo bài tập.
                </div>
              )}
            </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="text-base text-slate-800">
                            Nộp lần cuối:{" "}
                            {new Date(
                              sub.latestSubmissionTime ??
                                sub.firstSubmissionTime ??
                                Date.now()
                            ).toLocaleString()}
                          </div>
                ...
                <>
                  <div className="flex items-center justify-between">
                    <div className="text-base text-slate-600">
                      Đã nộp:{" "}
                      {new Date(
                        userSubmission.latestSubmissionTime ??
                          userSubmission.firstSubmissionTime ??
                          Date.now()
                      ).toLocaleString()}
                    </div>
                    <div>
                        ...
                        (f: ClassworkSubmissionFile) => (
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="w-10 h-10 bg-slate-100 rounded flex items-center justify-center text-sm">
                                📎
                              </div>
                  ...
                  <div className="text-sm text-slate-400">
                    Bạn có thể nộp lại nếu muốn thêm tệp.
                  </div>
                ...
                <>
                  <div className="relative" ref={menuRef}>
                    {allowSubmissionFlag ? (
                      ...
                      <div className="text-sm text-slate-500 p-2 border rounded">
                        Giáo viên đã tắt quyền nộp bài cho bài tập này.
                      </div>
                    )}
                  </div>

                  <div className="space-y-3 mt-4">
                    {files.length === 0 && linkAttachments.length === 0 ? (
                      <div className="text-slate-500 text-sm">
                        Chưa có tệp hoặc liên kết nào được thêm.
                      </div>
                    ) : (
                      <>
                        {files.map((f, idx) => (
                  ...
                  {allowSubmissionFlag ? (
                    <div className="flex gap-3 mt-4">
                      <Button
                        disabled={
                          isPastDeadline(classwork.deadline) || loadingSubmit
                        }
              ...
              <div className="flex items-start gap-4">
                <div className="w-12 h-12">
                  <Avatar className="w-12 h-12">
                    <AvatarFallback className="text-lg">
                      {gradedBy
                        ? String(gradedBy).charAt(0).toUpperCase()
                        : teacherForClasswork?.fullname
                        ? teacherForClasswork.fullname.charAt(0).toUpperCase()
                        : "G"}
                    </AvatarFallback>
                  </Avatar>
                </div>
                <div className="flex-1">
                  ...
                  <div className="mt-2 bg-slate-50 p-4 rounded-md text-sm text-slate-800 min-h-[48px]">
                    {teacherFeedback &&
                    String(teacherFeedback).trim().length > 0 ? (
                  ...
                  {(gradedBy || gradedAt) && (
                    <div className="mt-2 text-xs text-slate-400">
                      ...
                      {gradedAt ? (
                        <span>{new Date(gradedAt).toLocaleString()}</span>
                      ) : null}
                    </div>
                  )}
                </div>
