...
const detectFileType = (
  ...
  if (typeof fileOrName === "string") {
    ...
    if (/image\/(jpeg|png|webp|gif|bmp|svg)/i.test(fileOrName.type))
      return "image";
    if (
      /pdf/i.test(fileOrName.type) ||
      fileOrName.name.toLowerCase().endsWith(".pdf")
    )
      return "pdf";
    return "other";
  }
...
const AddClasswork: React.FC = () => {
  ...
  const { getClassInfo, getClassWorks } = useClassStore();
  ...
  console.log("cors", setInstructionsHtml);
      ...
      <div className="grid grid-cols-12 gap-6">
                          ...
                          {p.url && (
                            <button
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                void downloadUrl(p.url, p.file.name);
                              }}
        ...
        <aside className="col-span-12 lg:col-span-4">
          <Card className="p-5 space-y-4">
            ...
            <div>
              <Label>
                Điểm tối đa <span className="text-red-600 ml-1">*</span>
              </Label>
              <Input
                type="number"
                value={maxScore}
