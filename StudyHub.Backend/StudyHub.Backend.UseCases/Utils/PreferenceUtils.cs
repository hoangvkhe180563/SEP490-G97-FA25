...
namespace StudyHub.Backend.UseCases.Utils
{
    ...
    {
        public static List<SubjectPreference> CalculateSubjectPreferences(UserLearningProfile profile)
        {
            var preferences = new List<SubjectPreference>();

            foreach (var subject in profile.CurrentSubjectStudied)
            {
                var pref = new SubjectPreference { SubjectName = subject };
